...
//
// In the Label Text grid, Prefix and Suffix fields are live; the Value
// column is display-only (engine-generated).
//
...
pflabel_settings : dialog {
  ...
  // Prefix and Suffix are LIVE and wrap the engine values.  The Value column
  // is display-only (engine-generated: station / size+type+ID / elevation).
  ...
  // ---- Surface + centerlines ---------------------------------------------
  // TIN is one file (edit + Select).  Primary is one .cl (display + Select);
  // Select browses the file then prompts for the line name.  Secondaries are
  // a list (Add.../Remove); each row reads "NAME  (basename.cl)".  All three
  // are transient run inputs, not persisted.
  : boxed_column {
